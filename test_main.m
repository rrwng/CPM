...
close all
p=100;
data = [randn(100,p);randn(100,p)+3/sqrt(p);[0:1:99]'*1/sqrt(p)*ones(1,p)];
data = [data; 3*randn(100,p)+100/sqrt(p)];

C=[ones(100,1);ones(100,1)*2;ones(100,1)*3;ones(100,1)*4];
...
close all
N = 2^9; % number of points considered
t = rand(1,N);
t = sort(2*pi*sqrt(t))'; 
...
y = (t+.1).*sin(t);
w = .5*pi*randn(N,1);
data = [x,y,z,w,randn(N,1)*8,randn(N,1)*6];
...
figure(1);
subplot(2,2,1);scatter3(data(:,1),data(:,2),data(:,3),9,C,'filled'); title(' non-overlapping ball and shell: first two dimensions'); pause(.1);
ydata = compute_mapping(data,'PCA',2);
subplot(2,2,2); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('PCA');pause(.1);
ydata = compute_mapping(data,'tSNE',2);
subplot(2,2,3); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('tSNE');pause(.1);
...
data = test_X(k(1:1000),:); 
C = test_labels(k(1:1000));
figure(1); 
disp('Running PCA....')
ydata = compute_mapping(data,'PCA',2);
...
disp('Running LLE....')
ydata = compute_mapping(data,'LLE',2);
subplot(2,2,3); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('LLE');pause(.1);
disp('Running CPM with geodesic distance....')
ydata = cpm(data,2,1,4,1000);
figure(1);
subplot(2,2,4); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('CPM');
disp('press any key to run the next example')
pause;
...
disp('Running CPM....')
ydata = cpm(data,2,0,2,2000);
subplot(2,2,3); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('CPM with Euclidean distance');pause(.1);
disp('Running CPM with geodesic distance....')
ydata = cpm(data,2,1,2,1000);
subplot(2,2,4); scatter(ydata(:,1),ydata(:,2),9,C,'filled'); title('CPM with geodesic distance');
