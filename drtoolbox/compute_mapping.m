...
%     end
    ...
    switch type
        ...
        case {'Laplacian', 'LaplacianEig', 'LaplacianEigen' 'LaplacianEigenmaps'}
			...
			elseif length(varargin) == 1, [mappedA, mapping] = laplacian_eigen(A, no_dims,dist, varargin{1}, 1, eig_impl);
            elseif length(varargin) > 1,  [mappedA, mapping] = laplacian_eigen(A, no_dims, dist, varargin{1}, varargin{2}, eig_impl); end
        ...
        case {'DM', 'DiffusionMaps'}
			...
			if isempty(varargin), mappedA = diffusion_maps(A, no_dims,dist, 1, 1);
            elseif length(varargin) == 1, mappedA = diffusion_maps(A, no_dims,dist, varargin{1}, 1);
            else mappedA = diffusion_maps(A, no_dims, dist, varargin{1}, varargin{2}); end
            mapping.name = 'DM';
        ...
        case 'SNE'
            % Compute SNE mapping
			if isempty(varargin), mappedA = sne(A, no_dims,dist);
            else mappedA = sne(A, no_dims,dist, varargin{1}); end
            mapping.name = 'SNE';
...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
        ...
        case {'PCA', 'KLM'}
            % Compute PCA mapping
			[mappedA, mapping] = pca1(A, no_dims);
            mapping.name = 'PCA';
